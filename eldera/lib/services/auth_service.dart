...
class AuthService {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        try {
          ...
          final userId =
              profileData['id']?.toString() ?? (_currentUser?.id ?? '');
          final name =
              profileData['name']?.toString() ?? (_currentUser?.name ?? '');
          _currentUser = app_user.User(
            id: userId,
            name: name,
            age: age,
    ...
    if (token != null) {
      ...
      final needsProfile = _currentUser == null ||
          ...
          (_currentUser!.birthDate == null ||
              _currentUser!.birthDate!.isEmpty) ||
  ...
  static Future<Map<String, dynamic>> requestPasswordReset({
    ...
    try {
      ...
      SecureLogger.info(
          'Password reset response status: ${response.statusCode}');
      ...
      if (response.statusCode == 200) {
        ...
        try {
          ...
          return {
            'success': false,
            'message':
                data['message'] ?? 'Failed to submit password reset request',
          };
        } catch (e) {
          return {
            'success': false,
  ...
  static Future<Map<String, dynamic>> changePassword({
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('$_baseUrl/api/senior/change-password'),
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
          'Authorization': 'Bearer $token',
