...
class SecureStorageService {
  ...
  static Future<String?> getAuthToken() async {
    try {
      SecureLogger.info(
          'Attempting to retrieve auth token from secure storage');
      final tokenJson = await _storage.read(key: _keyAuthToken);

      if (tokenJson == null) {
        SecureLogger.info('No auth token found in secure storage');
        return null;
      }

      SecureLogger.info('Auth token found in secure storage, parsing...');
      ...
      if (tokenData['expires_at'] != null) {
        ...
        SecureLogger.info('Token expires at: $expiresAt, current time: $now');
