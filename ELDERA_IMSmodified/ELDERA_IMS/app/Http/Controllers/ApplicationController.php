    ...
    public function storePensionApplication(Request $request)
    {
        try {
            ...
            $data = [
                ...
                'age' => $request->age,
                'gender' => $request->gender,
                'civil_status' => $request->civil_status ?? 'Married', // Default to Married if not provided
            ...
            // Redirect to Social Pension table with success message
            ...
            \Log::error('Pension Application DB Error', [
            ...
            if (str_contains($e->getMessage(), 'cannot be null')) {
                if ($request->ajax()) {
                    return response()->json([
                        'success' => false,
                        'message' => 'Please fill out all required fields completely. Some information is missing.'
                    ], 422);
                }
                return redirect()->back()
                    ->with('error', 'Please fill out all required fields completely. Some information is missing.')
                ...
                if ($request->ajax()) {
                    return response()->json([
                        'success' => false,
                        'message' => 'This application already exists. Please check for duplicate entries.'
                    ], 422);
                }
                return redirect()->back()
                    ->with('error', 'This application already exists. Please check for duplicate entries.')
    ...
    public function storeBenefitsApplication(Request $request)
    {
        try {
            ...
            if ($request->senior_id) {
                ...
                if (!$senior || !$this->hasCompleteSeniorData($senior)) {
                    if ($request->ajax()) {
                        return response()->json([
                            'success' => false,
                            'message' => 'Cannot create benefits application. Senior data is incomplete. Please ensure all essential information is filled.'
                        ], 422);
                    }
                    return redirect()->back()
                ...
                if ($existingBenefitsApp) {
                    if ($request->ajax()) {
                        return response()->json([
                            'success' => false,
                            'message' => 'This senior already has an existing benefits application (ID: ' . $existingBenefitsApp->id . '). Please edit the existing application instead of creating a new one.'
                        ], 422);
                    }
                    return redirect()->back()
                ...
                if ($seniorAge < $milestoneAge) {
                    if ($request->ajax()) {
                        return response()->json([
                            'success' => false,
                            'message' => 'Invalid milestone age. Senior is ' . $seniorAge . ' years old but milestone age is ' . $milestoneAge . '. Milestone age cannot be higher than actual age.'
                        ], 422);
                    }
                    return redirect()->back()
            ...
            $data = [
                ...
                'age' => $request->age,
                // Address Information
                'res_house_number' => $request->res_house_number,
                'res_street' => $request->res_street,
                'res_barangay' => $request->res_barangay,
                'res_city' => $request->res_city,
                'res_province' => $request->res_province,
                'res_zip' => $request->res_zip,
                'perm_house_number' => $request->perm_house_number,
                ...
                'sex' => $request->sex,
                'civil_status' => $request->civil_status,
                'civil_status_others' => $request->civil_status_others,
                'citizenship' => $request->citizenship,
                'dual_citizenship_details' => $request->dual_citizenship_details,
                // Family Information
                'spouse_name' => $request->spouse_name,
                'spouse_citizenship' => $request->spouse_citizenship,
                'children' => $request->children,
                'authorized_reps' => $request->authorized_reps,
                // Contact Information
                'contact_number' => $request->contact_number,
                'email' => $request->email,
                // Beneficiaries
                'primary_beneficiary' => $request->primary_beneficiary,
                'contingent_beneficiary' => $request->contingent_beneficiary,
                // Utilization
                'utilization' => $request->utilization,
                'utilization_others' => $request->utilization_others,
                // Certification - Convert array to boolean
                'certification' => !empty($request->certification) && in_array('information_correct', $request->certification),
                // Validation Assessment
                'findings_concerns' => $request->findings_concerns,
            ...
            // Clear the benefits applications cache so new data shows immediately
            cache()->forget('benefits_applications_page_1');

            if ($request->ajax()) {
                return response()->json([
                    'success' => true,
                    'message' => 'Benefits application submitted successfully! Application ID: ' . $application->id,
                    'application_id' => $application->id
                ]);
            }

            // Always redirect back to the form with success message
            return redirect()->route('form_existing_senior')
