...
class ApplicationService
{
    ...
    public function createPensionApplication(array $data, array $files, int $uploadedBy): Application
    {
        return DB::transaction(function () use ($data, $files, $uploadedBy) {
            ...
            // Always use the senior's monthly income to ensure consistency
            $pensionApp = PensionApplication::create([
                'application_id' => $application->id,
                'senior_id' => $data['senior_id'],
                'rrn' => $data['rrn'] ?? null,
                'monthly_income' => $senior->monthly_income, // Use senior's monthly income instead of form data
    ...
    {
        // If senior_id is provided (existing senior), make personal fields optional
        $hasExistingSenior = !empty($data['senior_id']);
        
        $rules = [
            'senior_id' => 'nullable|integer|exists:seniors,id',
            'milestone_age' => 'required|integer|in:80,85,90,95,100',
            // Personal information - required only if no existing senior
            'last_name' => $hasExistingSenior ? 'nullable|string|max:255' : 'required|string|max:255',
            'first_name' => $hasExistingSenior ? 'nullable|string|max:255' : 'required|string|max:255',
            'middle_name' => 'nullable|string|max:255',
            'name_extension' => 'nullable|string|max:10',
            'date_of_birth' => $hasExistingSenior ? 'nullable|date' : 'required|date',
            'age' => $hasExistingSenior ? 'nullable|integer|min:0|max:150' : 'required|integer|min:0|max:150',
            'sex' => $hasExistingSenior ? 'nullable|string|in:Male,Female' : 'required|string|in:Male,Female',
            'civil_status' => $hasExistingSenior ? 'nullable|string|max:50' : 'required|string|max:50',
            'civil_status_others' => 'nullable|string|max:255',
            'citizenship' => $hasExistingSenior ? 'nullable|string|in:Filipino,Dual' : 'required|string|in:Filipino,Dual',
            'dual_citizenship_details' => 'nullable|string|max:500',
            'contact_number' => 'nullable|string|max:20',
            'email' => 'nullable|email|max:255',
            'osca_id' => $hasExistingSenior ? 'nullable|string|max:50' : 'required|string|max:50',
            'rrn' => 'nullable|string|max:50',
            // Address information
