                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-12">
                            ...
                            <div class="text-center mt-3">
                                 <button type="button" id="saveApplicationBtn" class="btn btn-danger px-5 py-2 fw-bold" style="background-color: #e31575; border-color: #e31575; border-radius: 20px;" onclick="window.confirmCreateBenefits()">SAVE APPLICATION</button>
                            </div>
                        </form>
                    </div>
                </div>
...
.btn-primary{
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Form validation will be handled in the AJAX submission handler below
        ...
        // Benefits Form Confirmation Handler (similar to pension form)
        window.confirmCreateBenefits = function() {
            ...
            if (!milestoneAge) {
                ...
                formMessages.classList.remove('d-none', 'alert-success', 'alert-info');
                formMessages.classList.add('alert-danger');
                formMessages.innerHTML = '<i class="fas fa-exclamation-circle"></i> Please select a milestone age.';
                return false;
            }
            ...
            if (!certification) {
                ...
                formMessages.classList.remove('d-none', 'alert-success', 'alert-info');
                formMessages.classList.add('alert-danger');
                formMessages.innerHTML = '<i class="fas fa-exclamation-circle"></i> Please check the certification checkbox to proceed.';
                return false;
            }
            ...
            // Build senior name from selected senior or form fields
            const selectedSeniorText = document.getElementById('senior-search')?.value || '';
            const firstName = document.querySelector('input[name="first_name"]')?.value || '';
            const lastName = document.querySelector('input[name="last_name"]')?.value || '';
            const seniorName = selectedSeniorText || (firstName + ' ' + lastName).trim() || 'this senior';

            // Show confirmation modal
            const title = 'Create Benefits Application';
            const message = `Are you sure you want to submit ${seniorName}'s benefits application for milestone age ${milestoneAge.value}?`;

            // Use shared modal if available; otherwise fallback to browser confirm
            if (typeof showConfirmModal === 'function') {
                showConfirmModal(title, message, '{{ route("forms.benefits.store") }}', 'POST');
            } else {
                if (confirm(message)) {
                    form.submit();
                }
